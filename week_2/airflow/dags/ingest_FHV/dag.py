...
from scripts import format_to_parquet, upload_to_gcs, format_to_parquet_chunks, format_to_parquet_pandas

PROJECT_ID = os.environ.get("GCP_PROJECT_ID")
BUCKET = os.environ.get("GCP_GCS_BUCKET")
BIGQUERY_DATASET = os.environ.get("BIGQUERY_DATASET", 'trips_data_all')

dataset_url_prefix = "https://s3.amazonaws.com/nyc-tlc/trip+data/fhv_tripdata_{dataset_month}.csv"
path_to_local_home = os.environ.get("AIRFLOW_HOME", "/opt/airflow/")

dataset_month = "{{ macros.ds_format(ds, '%Y-%m-%d', '%Y-%m') }}"
table_name = "FHV_{{ macros.ds_format(ds, '%Y-%m-%d', '%Y%m') }}"
...
with DAG(
    ...
    convert_task = PythonOperator(
        ...
        python_callable=format_to_parquet_chunks,
        op_kwargs={
